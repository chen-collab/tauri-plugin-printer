  ...
  <a-card>
    <div style="display: flex;flex-direction: column">
      <a-space style="margin-bottom: 10px">
        <a-button-group>
          <template v-for="tab in panelTabs">
            <a-button :type="curPanel === tab.key ? 'primary' : 'info'" @click="switchPanel(tab.key)" :key="tab.key">
              <span :class="tab.icon" style="margin-right: 4px"></span>
              {{ tab.name }}
            </a-button>
          </template>
        </a-button-group>
        <a-divider type="vertical" style="height: 28px" />
        <span style="font-size: 12px; color: #666">纸张:</span>
        ...
        <a-select v-model="selectedPrinter" placeholder="选择打印机" allowClear style="width: 180px">
          <a-select-option v-for="(p, i) in printerList" :key="i" :value="p">
            <a-icon type="printer" /> {{ p }}
          </a-select-option>
        </a-select>
        <a-button size="small" @click="loadPrinters" title="刷新打印机列表" style="margin-right: 8px">
          <a-icon type="reload" />
        </a-button>
        <a-button type="primary" icon="printer" @click="print" :loading="isPrinting">
          {{ isPrinting ? '打印中...' : '打印' }}
        </a-button>
      ...
      <a-space v-if="'1' == curKey" style="margin-bottom: 10px">
        ...
        <a-button type="primary" icon="printer" @click="print" :loading="isPrinting" :disabled="isPrinting">
          {{ isPrinting ? '打印中...' : '直接打印' }}
        </a-button>
...
// 新增模板
import panelPrescription from './panel-prescription.js'
import printDataPrescription from './print-data-prescription.js'
import panelInfusion from './panel-infusion.js'
import printDataInfusion from './print-data-infusion.js'
...
export default {
  ...
  data() {
    return {
      ...
      curPanel: 'default',
      panelTabs: [
        { key: 'default',        name: '默认模板',   icon: 'glyphicon glyphicon-file' },
        { key: 'prescription',   name: '处方签',     icon: 'glyphicon glyphicon-list-alt' },
        { key: 'infusion',       name: '输液标签',   icon: 'glyphicon glyphicon-tag' },
      ],
      // 打印机列表
      printerList: [],
      selectedPrinter: '',
  ...
  mounted() {
    // 检测是否运行在 Tauri 环境中
    this.isTauri = !!(window.__TAURI__ || window.__TAURI_INTERNALS__)
    this.getPanel()
    // 存在一个固定版本号，并且不是开发版本
    if (this.$parent.version && this.$parent.version != "development") {
      // 加载对应版本的 hiprint
  ...
  computed: {
     ...
     */
    currentPrintData() {
      if (this.curPanel === 'prescription') return printDataPrescription
      if (this.curPanel === 'infusion') return printDataInfusion
      return printData
    }
  },
  methods: {
    /**
     * @description: 加载 panel（支持处方签/输液标签模板）
     */
    getPanel() {
      // 内置模板：直接返回已导入的 panel
      if (this.curPanel === 'prescription') {
        panel = panelPrescription
        return
      }
      if (this.curPanel === 'infusion') {
        panel = panelInfusion
        return
      }
      // 默认模板：使用版本解析逻辑
    ...
    /**
     * @description: 切换模板 tab
     * @param {string} name 模板名称
     */
    switchPanel(name) {
      if (name === this.curPanel) return
      this.curPanel = name
      // 清空旧模板
      if (hiprintTemplate) {
        try { hiprintTemplate.clear() } catch (e) { /* 忽略 */ }
      }
      $('#hiprint-printTemplate').empty()
      // 加载新面板并重新初始化
      this.getPanel()
      if (hiprint && defaultElementTypeProvider) {
        this.init()
        // 根据模板自动设定纸张尺寸
        const p = panel.panels[0]
        if (p) {
          const w = p.width
          const h = p.height
          this.curPaper = { width: w, height: h, type: 'other' }
          if (w === 210 && h === 297) this.curPaper.type = 'A4'
          else if (w === 148 && h === 210) this.curPaper.type = 'A5'
          setTimeout(() => {
            try { hiprintTemplate.setPaper(w, h) } catch (e) { /* 忽略 */ }
          }, 100)
        }
      }
    },
    /**
     * @description: 加载打印机列表（Tauri 环境优先，浏览器回退）
     */
    async loadPrinters() {
      if (this.isTauri) {
        try {
          // 通过 Tauri invoke 直接调用打印机插件
          ...
          if (api) {
            ...
            if (typeof invoke === 'function') {
              const list = await invoke('plugin:printer|get_printers')
              if (Array.isArray(list)) {
                this.printerList = list.map(p => p.name || p)
              }
            }
          }
        } catch (e) {
          console.warn('Tauri 获取打印机失败:', e)
        }
      }
      // 浏览器备选：通过 hiprint（electron 客户端）获取
      if (!this.printerList.length && hiprintTemplate) {
        try {
          const list = hiprintTemplate.getPrinterList()
          if (list && list.length) {
            this.printerList = list.map(p => p.name || p)
          }
        } catch (e) { /* 忽略 */ }
    ...
    preView() {
      ...
      this.$refs.preView.show(hiprintTemplate, this.currentPrintData, width)
    },
    onlyPrint() {
      let hiprintTemplate = this.$print(undefined, panel, this.currentPrintData, {}, {
        styleHandler: () => {
    ...
    onlyPrint2() {
      let that = this;
      let theData = this.currentPrintData;
      if (window.hiwebSocket.opened) {
        let hiprintTemplate = this.$print2(undefined, panel, theData, {
    ...
    /**
     * @description: 打印（Tauri printHtml 优先，浏览器 hiprint 回退）
     */
    async tauriPrint() {
      if (!hiprintTemplate || this.isPrinting) return
      this.isPrinting = true
      try {
        // Tauri 环境：使用插件 printHtml
        if (this.isTauri) {
          await this.tauiPrintHtml()
          return
        }
        // 浏览器环境：优先使用 hiprint（electron 客户端），否则 window.print
        if (window.hiwebSocket && window.hiwebSocket.opened) {
          const printer = this.selectedPrinter || ''
          hiprintTemplate.print2(this.currentPrintData, {
            printer: printer,
            title: this.curPanel === 'prescription' ? '处方签打印' : '输液标签打印',
          })
          return
        }
        // 纯浏览器回退：使用浏览器打印
        await this.browserPrint()
      } catch (e) {
        this.$message.error('打印失败: ' + (e.message || e))
      } finally {
        this.isPrinting = false
      }
    },
    /**
     * @description: 通过 Tauri 插件 printHtml 打印
     */
    async tauiPrintHtml() {
      try {
        const htmlResult = hiprintTemplate.getHtml(this.currentPrintData)
        if (!htmlResult || !htmlResult.length) {
          this.$message.error('请先添加打印元素')
          return
        }
        ...
        // 通过 Tauri invoke 调用 printHtml 插件
        const api = window.__TAURI_INTERNALS__ || window.__TAURI__
        if (!api) throw new Error('Tauri 环境不可用')
        const invoke = typeof api.invoke === 'function'
          ? api.invoke
          : api.core?.invoke
        if (!invoke) throw new Error('无法获取 Tauri invoke')
        await invoke('plugin:printer|print_html', {
          options: {
            ...
            printerId: this.selectedPrinter || undefined,
        ...
        this.$message.error('Tauri 打印失败: ' + (e.message || e))
      }
    },
    /**
     * @description: 纯浏览器打印（打开新窗口打印）
     */
    async browserPrint() {
      const htmlResult = hiprintTemplate.getHtml(this.currentPrintData)
      if (!htmlResult || !htmlResult.length) {
    ...
    printByFragments() {
      // 分批打印仅在 electron 客户端可用，Tauri 环境下退化为普通打印
      if (this.isTauri) {
        this.tauriPrint()
        return
      }
      this.doOperationWhenClientConnected(() => {
        const dataList = new Array(50).fill(this.currentPrintData)
    ...
    doOperationWhenClientConnected(operation) {
      ...
      this.$error({
        ...
        content: (h) => (
          <div>
            连接【{hiwebSocket ? hiwebSocket.host : '未知'}】失败！
            <br/>
            请确保目标服务器已
            <a
              href="https://gitee.com/CcSimple/electron-hiprint/releases"
    ...
    exportPdf(type) {
      hiprintTemplate.toPdf(this.currentPrintData, '测试导出pdf', {isDownload: false, type: type}).then((res) => {
