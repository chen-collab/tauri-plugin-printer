...
fn map_printer_status(status: u32) -> Vec<String> {
    ...
    let pairs: &[(u32, &str)] = &[
        (0x00000001, "paused"), (0x00000002, "error"), (0x00000004, "pendingDeletion"),
        (0x00000008, "paperJam"), (0x00000010, "paperOut"), (0x00000020, "manualFeed"),
        (0x00000040, "paperProblem"), (0x00000080, "offline"), (0x00000100, "ioActive"),
        (0x00000200, "busy"), (0x00000400, "printing"), (0x00000800, "outputBinFull"),
        (0x00001000, "notAvailable"), (0x00002000, "waiting"), (0x00004000, "processing"),
        (0x00008000, "initializing"), (0x00010000, "warmingUp"), (0x00020000, "tonerLow"),
        (0x00040000, "noToner"), (0x00080000, "pagePunt"), (0x00100000, "userIntervention"),
        (0x00200000, "outOfMemory"), (0x00400000, "doorOpen"), (0x00800000, "serverUnknown"),
        (0x01000000, "powerSave"),
    ];
    for (mask, name) in pairs {
        if status & mask != 0 { result.push(name.to_string()); }
    }
    if result.is_empty() { result.push("normal".to_string()); }
    result
}
...
fn map_job_status(status: u32) -> Vec<String> {
    ...
    let pairs: &[(u32, &str)] = &[
        ...
        (0x00000200, "blocked"), (0x00000400, "userIntervention"), (0x00000800, "restart"),
    ];
    for (mask, name) in pairs {
        if status & mask != 0 { result.push(name.to_string()); }
    }
...
pub fn list_printers() -> Result<Vec<PrinterInfo>, Error> {
    ...
    // 第一次调用：获取缓冲区大小
    unsafe { let _ = EnumPrintersW(flags, None, 2, None, &mut needed, &mut returned); }
...
pub fn get_printer(name: &str) -> Result<PrinterInfo, Error> {
    ...
    let mut needed: u32 = 0;
    unsafe { let _ = GetPrinterW(handle.0, 2, None, &mut needed); }
    if needed == 0 { return Err(Error::WindowsApi(format!("printer '{}' not found", name))); }
    let mut buffer: Vec<u8> = vec![0u8; needed as usize];
    unsafe {
...
pub fn list_jobs(printer_name: &str) -> Result<Vec<JobInfo>, Error> {
    ...
    let mut needed: u32 = 0;
    let mut returned: u32 = 0;
    unsafe { let _ = EnumJobsW(handle.0, 0, 99, 1, None, &mut needed, &mut returned); }
    if needed == 0 { return Ok(Vec::new()); }
    let mut buffer: Vec<u8> = vec![0u8; needed as usize];
    unsafe {
        EnumJobsW(handle.0, 0, 99, 1, Some(&mut buffer[..]), &mut needed, &mut returned)
